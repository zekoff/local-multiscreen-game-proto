...
export function createCanvasRenderer({ container }) {
  ...
  function frame(ts) {
    ...
    // Cinematic feedback in fixed screen space. The CLEAN chrome (reticle,
    // off-screen chevron, notification/RED-ALERT/COLLISION banners) is drawn by
    // the shared hud-overlay, identical across renderers — not here.
    if (latest && latest.phase === 'active') {
    ...
    drawFlashes(w, h);
    drawFades(w, h, dpr);
    // Hull-breach instability flicker.
    drawGlitch(w, h);
